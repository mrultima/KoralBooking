...
export class RoomsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(public apiService: ApiService, private basketService: BasketService) {
  }
  ...
  roomsToRoomList(rooms: Rooms): RoomList {
    ...
    rooms.forEach(room => {
      room._PriceBlock = JSON.parse(room.PriceBlock);
  ...
  async onSelectRoom(room: any, count: number) {
    // console.log(this.roomList$, "roomList");
    this.basketService.selectRoom(room._rooms[0], count);
  }
